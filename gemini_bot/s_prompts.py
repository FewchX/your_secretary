...
Пользователь настраивает свой часовой пояс, чтобы бот показывал время в его регионе.

Задача:
1. Определи разницу между локальным временем сервера (server_time) и временем, которое назвал пользователь.
2. Вычисли смещение UTC в формате ±HH:MM.
3. Сохрани это смещение в поле utc_offset таблицы Users.
...
Выход (JSON):
{
  ...
  "sql": "UPDATE Users SET utc_offset = '{offset}' WHERE id = {USER_ID};"
}

Правила:
- Смещение округлять до минут.
- Если пользователь сказал только время, вычислить разницу с server_time.
"""
